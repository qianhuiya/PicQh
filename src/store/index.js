...
import {PICQH_CONFIG, PICQH_MANAGEMENT, PICQH_UPLOADED} from "../common/model/localStorage";
import cleanObject from "../common/utils/cleanObject";

Vue.use(Vuex)

const initUserConfigInfo = () => {
  let config = localStorage.getItem(PICQH_CONFIG)
  return config
    ? JSON.parse(config)
    : {
      token: '',
...
const initDirImageList = () => {
  let dirImageList = localStorage.getItem(PICQH_MANAGEMENT)
  return dirImageList ? JSON.parse(dirImageList) : []
}

const initUploadedImageList = () => {
  let imageList = sessionStorage.getItem(PICQH_UPLOADED)
...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    PERSIST_USER_CONFIG_INFO(state) {
      ...
      localStorage.setItem(PICQH_CONFIG, JSON.stringify(state.userConfigInfo))
    },
  ...
  actions: {
    ...
    LOGOUT({commit, state}) {
      ...
      localStorage.removeItem(PICQH_CONFIG)
      localStorage.removeItem(PICQH_MANAGEMENT)
      sessionStorage.removeItem(PICQH_UPLOADED)
    },
    ...
    DIR_IMAGE_LIST_PERSIST({state}) {
      localStorage.setItem(PICQH_MANAGEMENT, JSON.stringify(state.dirImageList))
    },
    ...
    UPLOADED_LIST_PERSIST({state}) {
      sessionStorage.setItem(PICQH_UPLOADED, JSON.stringify(state.uploadedImageList))
    },
  },
  modules: {}
})
